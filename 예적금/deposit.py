# 예금 API URL
def depositProductsSearch(auth, topFinGrpNo, pageNo, financeCd):
  if financeCd:
    return "http://finlife.fss.or.kr/finlifeapi/depositProductsSearch.json?auth={}&topFinGrpNo={}&pageNo={}&financeCd={}".format(auth, topFinGrpNo, pageNo, financeCd)
  else:
    return "http://finlife.fss.or.kr/finlifeapi/depositProductsSearch.json?auth={}&topFinGrpNo={}&pageNo={}".format(auth, topFinGrpNo, pageNo)
...
for grpCode in topFinGrpNo:
  ...
  page = 1
  while True:
    url = depositProductsSearch(auth, grpCode, page, "")
    try:
    ...
    for o in option:
      ...
      product[grpCode][code]["options"].append([o["save_trm"], o["intr_rate_type_nm"], o["intr_rate"], o["intr_rate2"]])
...
# product["권역코드"]["상품코드"]["옵션"][0] = {"저축기간", "금리종류", "금리", "최대 금리"}